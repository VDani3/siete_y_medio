        ...
        while roundgame < rounds and len(active_players) > 1:
            ...
            for id in active_players:                                                              #Empieza la ronda de los jugadores NO banca
                if players_game[id]["human"] == False and players_game[id]["bank"] == False:       #Si es Bot
                    ...
                    players_game[id] = roundPlayer(players_game[id],avaliable_cards)
                    avaliable_cards = removeCardsFromDeck(players_game[id],avaliable_cards)
                    passed = True
            for banca in active_players:
                if players_game[banca]["human"] == False and players_game[banca]["bank"] == True:   #Si es bot y Banca
                    ...
                    print("hola")
                    players_game[banca] = roundPlayer(players_game[banca],avaliable_cards)
            ...
            for printear in players_game_keys:
               print( str(players_game[printear]["name"])+printear+" points have: "+str(players_game[printear]["points"])+ " c: "+str(players_game[printear]["cards"])+" rounPoints: "+str(players_game[printear]["roundPoints"]) + "     "+str(players_game[printear]["bank"]))
            input()
            clear()
            roundgame += 1
        checkWinner(active_players, players_game) 
        input()
        input()
        flg_00 = True
        flg_03 = False
