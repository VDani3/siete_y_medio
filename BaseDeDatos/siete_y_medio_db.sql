...
CREATE TABLE player_game_round (
	...
	FOREIGN KEY (cardgame_id) REFERENCES cardgame(cardgame_id),
    	round_num INT UNSIGNED AUTO_INCREMENT ,
...
drop view minutes_played;
/*Creacion de las vistas*/
CREATE VIEW profits as
select p.player_id, p.player_name, pg.ending_points-pg.starting_points as profit
from player p, player_game pg
where p.player_id = pg.player_id;

CREATE VIEW games_played as
select p.player_id, p.player_name, count(pg.player_id) as games_played
from player p, player_game pg
where p.player_id = pg.player_id
group by 1,2;

CREATE VIEW minutes_played as
select p.player_id, p.player_name, (TIMEDIFF(end_hour,start_hour)) as minutes_played
from player p, player_game pg, cardgame c
where p.player_id = pg.player_id and 
		c.cardgame_id = pg.cardgame_id
group by 1,2,3;
